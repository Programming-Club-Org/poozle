...
std::vector<int> expected_output(const std::string &filepath) {
  ...
  if (!file.is_open()) {
    std::cerr << "Error opening expected output file: " << filepath
              << std::endl;
    exit(1);
  }
  int val;
...
int main(int argc, char *argv[]) {
  ...
  while (std::getline(input_file, line)) {
    ...
    if (line == "QUERY") {
      if (text_pres) {
        ...
        auto build_duration =
            std::chrono::duration_cast<std::chrono::microseconds>(build_end -
                                                                  build_start)
                .count();
        std::cout << "Time taken for building fm index : " << build_duration
                  << " us" << std::endl;
        text_pres = 0;
      }
      ...
      auto q_duration =
          std::chrono::duration_cast<std::chrono::microseconds>(q_end - q_start)
              .count();
      ...
      if (result == expected) {
        std::cout << "Tests passed. Time taken: " << q_duration << " us"
                  << std::endl;
        if (expected_idx == expected_outputs.size())
          std::cout << "All tests passed." << std::endl;
      } else {
        std::cout << "False (Expected " << expected << ", Got " << result << ")"
                  << std::endl;
        delete fm_index;
        return 0;
      }
